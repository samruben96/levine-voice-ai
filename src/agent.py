...
- **CancellationAgent**: Specialized agent for policy cancellation requests.
  Shows empathy without being pushy about retention, collects insurance type
  and identifier, then routes to Account Executives via alpha-split.
  Supports smart context detection for business vs personal insurance.
...
# =============================================================================
# CANCELLATION AGENT - Handles policy cancellation flow
# =============================================================================
...
2. DETERMINE TYPE from context clues (SMART DETECTION):
   - Business clues: "work policy", "company", "LLC", "commercial", "business policy", "office insurance" -> This is BUSINESS insurance, confirm with them
   - Personal clues: "car", "home", "auto", "my policy", "family", "vehicle" -> This is PERSONAL insurance, confirm with them
   - If unclear: ask "Is this for your business or personal insurance?"
   - IMPORTANT: These are CLUES, not the business name!

3. COLLECT INFO (always ask - never assume):
   - BUSINESS: "What is the name of the business?" (wait for answer)
   - PERSONAL: "Can you spell your last name for me?"
   Use record_business_cancellation_info or record_personal_cancellation_info after they answer.
...
- No longer need the coverage""",
    ...
    async def on_enter(self) -> None:
        ...
        self.session.generate_reply(
            instructions="""Start with brief empathy, then check if the caller already indicated business or personal context:
...
- If clear from context, confirm with them. If unclear, ask: 'Is this for your business or personal insurance?'"""
    ...
    async def _initiate_transfer(
        ...
        caller_phone = context.userdata.phone_number
        logger.info(
            f"[MOCK TRANSFER] Initiating cancellation transfer to {agent_name} (ext {agent_ext}) for caller: "
            f"name={mask_name(caller_name) if caller_name else 'unknown'}, "
            f"phone={mask_phone(caller_phone) if caller_phone else 'unknown'}"
        )
    ...
    async def _handle_fallback(
        ...
        TODO (Needs Client Input): Determine fallback behavior. Options:
        - Option A (CURRENT): Take a data sheet for callback
        - Option B: Ring all Account Executives in the department simultaneously
        - Option C: Ring a specific backup Account Executive

        Current behavior: Option A - Take a data sheet for callback.
        """
        if unavailable_agent:
            logger.info(
                f"Agent {unavailable_agent} unavailable for cancellation, using fallback: take_data_sheet"
            )
        else:
            logger.info(
                "No agent assigned for cancellation, using fallback: take_data_sheet"
            )
    ...
    async def _take_data_sheet(self, context: RunContext[CallerInfo]) -> str:
        ...
        Logs all relevant cancellation information for callback purposes:
        - Name, phone number (from context.userdata)
        - Insurance type (business/personal)
        - Business name / last name spelling
        - Reason for cancellation (if provided, stored in additional_notes)

        TODO (Needs Client Input): Determine hold timeout before this fallback triggers.
        """
        userdata = context.userdata
        logger.info(
            f"Taking data sheet for cancellation callback: "
            f"name={mask_name(userdata.name) if userdata.name else 'unknown'}, "
            f"phone={mask_phone(userdata.phone_number) if userdata.phone_number else 'unknown'}, "
            f"type={userdata.insurance_type}, "
            f"business={userdata.business_name}, "
...
- OTHER REQUESTS: Use route_call with intent: coverage_questions, policy_review, mortgagee_lienholder, certificates, claims, or other
