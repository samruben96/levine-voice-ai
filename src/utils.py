...
def format_email_for_speech(email: str) -> str:
    """Format email for TTS: read normally first, then spell it out.

    Example: 'info@hlinsure.com' -> 'info@hlinsure.com. I'll spell that out: I-N-F-O at H-L-I-N-S-U-R-E dot com.'
    """
    if not email or "@" not in email:
        return email
    ...
    return (
        f"{email}. I'll spell that out: {local_spelled} at {domain_spelled} dot {tld}."
    )
